...
namespace lucy\LucyMail\Test\Unit\Block\Adminhtml\System\Config\Fieldset;
...
class HintTest extends \PHPUnit_Framework_TestCase
{
    ...
    protected function setUp()
    {
        ...
        $loaderMock = $this->getMockBuilder('Magento\Framework\Module\ModuleList\Loader')
            ->disableOriginalConstructor()
            ->getMock();
        $loaderMock->expects($this->any())->method('load')->willReturn(['lucy_LucyMail'=> ['setup_version'=>3]]);
        $objectManager = new \Magento\Framework\TestFramework\Unit\Helper\ObjectManager($this);
        $this->_hint = $objectManager->getObject(
            'lucy\LucyMail\Block\Adminhtml\System\Config\Fieldset\Hint',
            [
                'context' => $contextMock,
    ...
    {
        $this->assertEquals($this->_hint->getPxParams(), 'ext=LucyMail;3&mage=Magento CE;1&ctrl=818b95f17fc2d5e4bc1560681f1eb287');
        $this->assertEquals($this->_hint->getPxParams(), 'ext=LucyMail;3&mage=Magento EE;1&ctrl=21280f46ed6d9789a031f06e0ce5767c');
    }
